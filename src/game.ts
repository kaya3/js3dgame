...
class Game {
	...
	public tick(dt: number, keys: { [k: number]: number }): void {
        ...
        if(keys[KEYCODE_PAGE_UP]) { this.camera.scale = Util.limitNumberRange(this.camera.scale + dz, Game.MIN_ZOOM, Game.MAX_ZOOM); } // page up/zoom in
        if(keys[KEYCODE_PAGE_DOWN]) { this.camera.scale = Util.limitNumberRange(this.camera.scale - dz, Game.MIN_ZOOM, Game.MAX_ZOOM); } // page down/zoom out

		const stepSize = 0.1;
        if(keys[KEYCODE_A]) { this.movePlayerWithinBounds(SCENE_DATA.player, -stepSize, -stepSize); }
        else if(keys[KEYCODE_D]) { this.movePlayerWithinBounds(SCENE_DATA.player, +stepSize, +stepSize); }
        else if(keys[KEYCODE_W]) { this.movePlayerWithinBounds(SCENE_DATA.player, +stepSize, -stepSize); }
        else if(keys[KEYCODE_S]) { this.movePlayerWithinBounds(SCENE_DATA.player, -stepSize, +stepSize); }
	}

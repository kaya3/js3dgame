...
class Game {
    ...
    public tick(dt: number, keys: { [k: number]: number }): void {
        ...
        const stepSize = 0.1;
        if (keys[KEYCODE_A]) { this.movePlayerWithinBounds(SCENE_DATA.player, -stepSize, -stepSize); } else if (keys[KEYCODE_D]) { this.movePlayerWithinBounds(SCENE_DATA.player, +stepSize, +stepSize); } else if (keys[KEYCODE_W]) { this.movePlayerWithinBounds(SCENE_DATA.player, +stepSize, -stepSize); } else if (keys[KEYCODE_S]) { this.movePlayerWithinBounds(SCENE_DATA.player, -stepSize, +stepSize); }
    }
